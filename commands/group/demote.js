...
module.exports = {
	...
	async run({ msg, naisa }, { q, prefix }) {
	  try{
		...
		if (!add) return msg.reply("Example: " + prefix + "demote 62xxxxxx");
		...
		let participant = await naisa.groupParticipantsUpdate(msg.chat, chunk, "demote").then((res) => msg.reply(util.format(res))).catch((err) => msg.reply(util.format(err)))
	  } catch (err) {
	}
