...
module.exports = {
	...
	async run({ msg, naisa }, { q, prefix }) {
	  try{
		...
		if (!add) return msg.reply("Example: " + prefix + "add 62xxxxxxxx");
		...
		let participant = await naisa.groupParticipantsUpdate(msg.chat, chunk, "add").catch((err) => msg.reply("ERROR TIDAK BISA MENAMBAHKAN USER BARU"));
		await require("delay")(5000);
		const cek = await naisa.groupMetadata(msg.chat);
