...
module.exports = {
	...
	noLimit: true,
    start: async(naisa, m, {isPremium, isOwner}) => {
		let statuses
		try {
			statuses = await naisa.fetchStatus(m.sender);
		} catch {
			statuses = "Nothing.."
		}
		...
		try {
			var pp = await naisa.profilePictureUrl(m.sender, "image");
		} catch {
		...
		caption += `│\n`
		caption += `└───────⭓\n`
		naisa.sendFile(m.from, pp, "", m, { caption })
    }
}