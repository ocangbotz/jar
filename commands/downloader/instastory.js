...
module.exports = {
    ...
    start: async(naisa, m, { text }) => {
        if (isUrl(text)) {
            ...
            naisa.sendFile(m.from, fetch.result[0], "", m, { caption: `Download Story From : ${isUrl(text)[0]}` })
        } else {
            let fetch = await fetchUrl(global.api("zenz", "/downloader/instagram/story", { username: text }, "apikey"))
            for (let i of fetch.result.url_list) naisa.sendFile(m.from, i, "", m, { caption: `Download Story From : ${text}` })
        }
    },
    isQuery: true
}
