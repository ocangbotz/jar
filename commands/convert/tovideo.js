...
module.exports = {
    ...
    start: async(naisa, m, { command, prefix, quoted, mime, text }) => {
        if (/image|video|sticker/.test(mime)) {
            let download = await naisa.downloadMediaMessage(quoted)
            ...
            axios.post(global.api("zenz", "/convert/webp-to-mp4", {}, "apikey"), form.getBuffer(), { headers: { "content-type": `multipart/form-data; boundary=${form._boundary}`}
                ...
                naisa.sendFile(m.from, data.result, "", m, { caption: 'Convert Sticker Gif To Video' })
            })
        } else if (isUrl(text)) {
            let fetch = await fetchUrl(global.api("zenz", "/convert/webp-to-mp4", { url: isUrl(text)[0] }, "apikey"))
            naisa.sendFile(m.from, fetch.result, "", m, { caption: 'Convert Sticker Gif To Video' })
        }   else {
